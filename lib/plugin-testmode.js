...
scimgateway.on('getUser', function (baseEntity, userName, attributes, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "getUser" userName=' + userName + ' attributes=' + attributes);
    var retObj = {};
...
scimgateway.on('createUser', function (baseEntity, userObj, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "createUser" userObj=' + JSON.stringify(userObj));
...
scimgateway.on('deleteUser', function (baseEntity, id, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "deleteUser" id=' + id);
...
scimgateway.on('modifyUser', function (baseEntity, id, attrObj, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "modifyUser" id=' + id + ' attrObj=' + JSON.stringify(attrObj));
...
scimgateway.on('getGroupMembers', function (baseEntity, id, attributes, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "getGroupMembers" user id=' + id + ' attributes=' + attributes);
    var arrRet = [];
...
//             GET USERS GROUP
//
// retrieve all users for a spesific group
...
// If we do not support groups, callback(null, []) with no additional code lines
//==========================================
scimgateway.on('getUsersGroup', function (baseEntity, groupName, attributes, callback) {
    scimgateway.logger.debug(pluginName + ' handling event "getUsersGroup" groupName=' + groupName + ' attributes=' + attributes);
    var arrRet = [];
