...
const ScimGateway = function () {
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    logger.debug(`${gwName}[${pluginName}] [Get ${handle.description}] ${getObj.attribute}=${getObj.value}`)
    logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)

    try {
      const res = await this[handle.getMethod](ctx.params.baseEntity, utils.copyObj(getObj), ctx.query.attributes ? ctx.query.attributes : '')
      ...
      if (handle.getMethod === handler.users.getMethod && Object.keys(userObj).length > 0 && !userObj.groups && !config.scim.groupMemberOfUser) { // groupMemberOfUser can be set to true for skipping
        ...
        if ((!ctx.query.attributes || arrAttr.includes('groups')) && typeof handler.groups.getMethod === 'function') { // include groups
          logger.debug(`${gwName}[${pluginName}] calling "${handler.groups.getMethod}" and awaiting result - groups to be included`)
          let res
          try {
            res = await this[handler.groups.getMethod](ctx.params.baseEntity, { attribute: 'members.value', operator: 'eq', value: getObj.value }, 'members.value,id,displayName') // getUserGroups
  ...
  router.get(['/(|scim/)(Users|Groups|servicePlans)',
    ...
    } else if (getObj.rawFilter && ![' and ', ' or ', ' not '].some(el => getObj.rawFilter.includes(el))) {
    ...
    //   simpel filtering
    //   advanced filtering
    //   => one or more users to be returend
    // ==========================================
    //
    ...
    // GET /Users /Groups
    // GET /Users?filter=meta.created gte "2010-01-01T00:00:00"&attributes=userName,id,name.givenName
    // GET /Useers?filter=name.
    logger.debug(`${gwName}[${pluginName}] [Get ${handle.description}s]`)
    logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
    try {
    ...
    if (strBody === '{}') {
      ...
      try {
        ...
        if (ctx.query.attributes || ctx.query.excludedAttributes) {
          ...
          const res = await this[handle.getMethod](ctx.params.baseEntity, { attribute: 'id', operator: 'eq', value: id }, ctx.query.attributes ? ctx.query.attributes : '')
          let scimdata = {
            Resources: [],
            totalResults: null
          }
