...
const ScimGateway = function () {
  ...
  const getById = async (ctx, next) => {
    ...
    try {
      ...
      if (config.stream.publisher.enabled) {
        ...
        res = await this[handle.getMethod](ctx.params.baseEntity, ob, attributes, ctx.ctxCopy)
      }
        ...
        if ((!ctx.query.attributes || arrAttr.includes('groups')) && typeof this[handler.groups.getMethod] === 'function') { // include groups
          ...
          try {
            ...
            if (config.stream.publisher.enabled) {
              ...
              res = await this[handler.groups.getMethod](ctx.params.baseEntity, ob, attributes, ctx.ctxCopy)
            }
          ...
          for (let j = 0; j < scimdata.Resources.length; j++) {
            ...
            try {
              ...
              if (config.stream.publisher.enabled) {
                ...
                logger.debug(`${gwName}[${pluginName}] calling "${handler.groups.getMethod}" and awaiting result - groups to be included`)
                res = await this[handler.groups.getMethod](ctx.params.baseEntity, ob, attributes, ctx.ctxCopy) // await scimgateway.getUserGroups(baseEntity, userObj.id, 'members.value,displayName')
              }
            } catch (err) {} // method may be implemented but throwing error like groups not supported/implemented
            if (res && res.Resources && Array.isArray(res.Resources) && res.Resources.length > 0) {
              userObj.groups = []
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`], async (ctx) => {
    ...
    try {
      ...
      if (config.stream.publisher.enabled) {
        ...
        res = await this[handle.createMethod](ctx.params.baseEntity, scimdata, ctx.ctxCopy)
      }
      ...
      if (addGrps.length > 0 && handle.createMethod === 'createUser') { // add group membership
        const addGroups = async (groupId) => { 
          if (config.stream.publisher.enabled) {
            ...
            return await this[handler.groups.modifyMethod](ctx.params.baseEntity, groupId, { members: [{ value: jsonBody.id }] }, ctx.ctxCopy)
          }
        }
        ...
        if (groups.length > 0 && handle.modifyMethod === 'modifyUser') { // modify user includes groups, add/remove group membership
          const updateGroup = async (groupsObj) => { 
            ...
            if (config.stream.publisher.enabled) {
              ...
              return await this[handler.groups.modifyMethod](ctx.params.baseEntity, groupId, { members: [ memberObj ] }, ctx.ctxCopy)
            }
          }
  ...
  const replaceUsrGrp = async (ctx) => {
    ...
    // get current object
    logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
    const res = await this[handle.getMethod](ctx.params.baseEntity, { attribute: 'id', operator: 'eq', value: id }, [], ctx.ctxCopy)
    let currentObj
    ...
    if (!config.scim.groupMemberOfUser || !config.scim.usePutGroupMemberOfUser) { // default user member of group, usePutGroupMemberOfUser is legacy
      if (objGroups && Array.isArray(objGroups)) { // only if groups included, { "groups": [] } will remove all existing
        ...
        const addGroups = async (grp) => {
          return await this[handler.groups.modifyMethod](ctx.params.baseEntity, grp, { members: [{ value: id }] }, ctx.ctxCopy)
        }

        const removeGroups = async (grp) => {
          return await this[handler.groups.modifyMethod](ctx.params.baseEntity, grp, { members: [{ operation: 'delete', value: id }] }, ctx.ctxCopy)
        }
    ...
    if (strBody === '{}') {
      ...
      try {
        ...
        if (config.stream.publisher.enabled) {
          ...
          result = await this.postApi(ctx.params.baseEntity, apiObj, ctx.ctxCopy)
        }
        if (result) {
    ...
    if (strBody === '{}') {
      ...
      try {
        ...
        if (config.stream.publisher.enabled) {
          ...
          result = await this.putApi(ctx.params.baseEntity, id, apiObj, ctx.ctxCopy)
        }
        if (result) {
          if (typeof result === 'object') result = { result: result }
          else {
    ...
    if (strBody === '{}') {
      ...
      try {
        ...
        if (config.stream.publisher.enabled) {
          ...
          result = await this.patchApi(ctx.params.baseEntity, id, apiObj, ctx.ctxCopy)
        }
        if (result) {
          if (typeof result === 'object') result = { result: result }
          else {
  ...
  router.get(['/api', '/api/:id',
    ...
    try {
      ...
      if (config.stream.publisher.enabled) {
        ...
        result = await this.getApi(ctx.params.baseEntity, id, ctx.query, apiObj, ctx.ctxCopy)
      }
      if (result) {
        if (typeof result === 'object') result = { result: result }
        else {
  ...
  router.delete(['/api/:id', '/:baseEntity/api/:id'], async (ctx) => {
    ...
    try {
      ...
      if (config.stream.publisher.enabled) {
        ...
        result = await this.deleteApi(ctx.params.baseEntity, id, ctx.ctxCopy)
      }
      if (result) {
        if (typeof result === 'object') result = { result: result }
        else {
