...
ScimGateway.prototype.convertedScim = function convertedScim (obj) {
  ...
  for (const key in scimdata) {
    ...
    } else if (key.includes(':1.0:') || key.includes(':2.0:')) { // "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {...}
...
const convertedScim20 = (data) => {
  ...
  data.Operations.forEach(function (element, index) {
    ...
    if (element.path) {
      ...
      if (element.path.includes(':2.0:')) { // {"path": "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User:department"}
