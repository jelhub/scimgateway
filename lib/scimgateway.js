    ...
    if (strBody === '{}') {
      ...
      try {
        if (Array.isArray(scimdata.members) && scimdata.members.length === 0 && handle.modifyMethod === 'modifyGroup') {
          let ctxr = ctx.ctxCopy
          if (!ctxr) ctxr = {}
          ctxr.set = () => {}
          ctxr.params = {}
          ctxr.params.baseEntity = ctx.params.baseEntity
          ctxr.params.id = id
          ctxr.request = {}
          ctxr.request.originalUrl = ctx.params.baseEntity ? `/${ctx.params.baseEntity}/Groups/${id}` : `/Groups/${id}`
          ctxr.request.body = scimdata
          await replaceUsrGrp(ctxr)
          ctx.status = ctxr.status
          ctx.body = ctxr.body
          return
        } else {
