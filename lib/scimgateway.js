      ...
      function (err, results) { // final callback
        ...
        if (!isAuthenticated && !err) {
          ...
          if (req.url !== '/favicon.ico') {
            ...
            sendMail(config.emailOnError, `${gwName} ${err.message}`)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
          }
          ...
          if (pwErrCount < 3) {
            ...
            sendMail(config.emailOnError, `${gwName} ${req.url} ${err.message}`)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
            ...
            sendMail(config.emailOnError, `${gwName} ${req.url} ${err.message} => delaying response with 2 minutes to prevent brute force`)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
          }
        } else {
  ...
  app.get(`(|/:baseEntity)(|/scim)/(!${undefined}|Users|Groups|servicePlans)/:id`, (req, res) => {
    ...
    this.emit(handle.getEmitter, req.params.baseEntity, id, req.query.attributes ? req.query.attributes : '', function (err, data) {
      if (err) {
        err = jsonErr(config.scimversion, pluginName, '404', err)
        res.status(404).send(err)
        let logMsg = `${gwName}[${pluginName}] GET = ${req.originalUrl} Response = ${JSON.stringify(err)}`
        logger.error(logMsg)
        sendMail(config.emailOnError, logMsg)
          .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
          .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
      } else {
        let protocol = 'http'
    ...
    if (!req.query.filter) {
      ...
      if (arrFilter.length > 2 && arrFilter[1] === 'eq') {
        if ((arrFilter[0] === handle.uniqueAttr || arrFilter[0] === 'id' || arrFilter[0] === 'externalId' || arrFilter[0] === 'members')) {
          ...
          let logMsg = `${gwName} GET = ${req.originalUrl} Response = ${JSON.stringify(err)}`
          logger.error(logMsg)
          sendMail(config.emailOnError, logMsg)
            .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
            .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
      ...
      if (obj === null) {
        ...
        this.emit(handle.createEmitter, req.params.baseEntity, obj, function (err) {
          if (err) {
            err = jsonErr(config.scimversion, pluginName, '500', err)
            res.status(500).send(err)
            let logMsg = `${gwName}[${pluginName}] POST = ${req.originalUrl} Response = ${JSON.stringify(err)}`
            logger.error(logMsg)
            sendMail(config.emailOnError, logMsg)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
  ...
  app.delete('(|/:baseEntity)(|/scim)/Users/:id', (req, res) => {
    ...
    this.emit('deleteUser', req.params.baseEntity, id, function (err) {
      if (err) {
        err = jsonErr(config.scimversion, pluginName, '500', err)
        res.status(500).send(err)
        let logMsg = `${gwName}[${pluginName}] DELETE = ${req.originalUrl} Response = ${JSON.stringify(err)}`
        logger.error(logMsg)
        sendMail(config.emailOnError, logMsg)
          .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
          .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
    ...
    req.on('end', () => {
      ...
      this.emit(handle.modifyEmitter, req.params.baseEntity, id, scimdata.members ? scimdata.members : scimdata, function (err) {
        if (err) {
          err = jsonErr(config.scimversion, pluginName, '500', err)
          res.status(500).send(err)
          let logMsg = `${gwName}[${pluginName}] PATCH = ${req.originalUrl} Response = ${JSON.stringify(err)}`
          logger.error(logMsg)
          sendMail(config.emailOnError, logMsg)
            .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
            .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
    ...
    req.on('end', () => {
      ...
      this.emit(handle.modifyEmitter, req.params.baseEntity, id, scimdata.members ? scimdata.members : scimdata, function (err) {
        if (err) {
          err = jsonErr(config.scimversion, pluginName, '500', err)
          res.status(500).send(err)
          let logMsg = `${gwName}[${pluginName}] PUT = ${req.originalUrl} Response = ${JSON.stringify(err)}`
          logger.error(logMsg)
          sendMail(config.emailOnError, logMsg)
            .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
            .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
      ...
      if (apiObj === null) {
        ...
        this.emit('postApi', req.params.baseEntity, apiObj, function (err, result) {
          if (err) {
            ...
            res.status(500).send(errResult)
            let logMsg = `${gwName}[${pluginName}] POST = ${req.originalUrl} Response = ${JSON.stringify(errResult)}`
            logger.error(logMsg)
            sendMail(config.emailOnError, logMsg)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
      ...
      if (apiObj === null) {
        ...
        this.emit('putApi', req.params.baseEntity, id, apiObj, function (err, result) {
          if (err) {
            ...
            res.status(500).send(errResult)
            let logMsg = `${gwName}[${pluginName}] PUT = ${req.originalUrl} Response = ${JSON.stringify(errResult)}`
            logger.error(logMsg)
            sendMail(config.emailOnError, logMsg)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
      ...
      if (apiObj === null) {
        ...
        this.emit('patchApi', req.params.baseEntity, id, apiObj, function (err, result) {
          if (err) {
            ...
            res.status(500).send(errResult)
            let logMsg = `${gwName}[${pluginName}] PATCH = ${req.originalUrl} Response = ${JSON.stringify(errResult)}`
            logger.error(logMsg)
            sendMail(config.emailOnError, logMsg)
              .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
              .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
  ...
  app.get('(|/:baseEntity)/api(|/:id)', (req, res) => {
    ...
    this.emit('getApi', req.params.baseEntity, id, req.query, function (err, result) {
      if (err) {
        ...
        let logMsg = `${gwName}[${pluginName}] GET = ${req.originalUrl} Response = 404 ${JSON.stringify(errResult)}`
        logger.error(logMsg)
        sendMail(config.emailOnError, logMsg)
          .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
          .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
  ...
  app.delete('(|/:baseEntity)/api/:id', (req, res) => {
    ...
    this.emit('deleteApi', req.params.baseEntity, id, function (err, result) {
      if (err) {
        ...
        res.status(500).send(errResult)
        let logMsg = `${gwName}[${pluginName}] DELETE = ${req.originalUrl} Response = 500 ${JSON.stringify(errResult)}`
        logger.error(logMsg)
        sendMail(config.emailOnError, logMsg)
          .then(sentTo => { if (sentTo) logger.debug(`${gwName} mailOnError sent to: ${sentTo}`) })
          .catch(err => { logger.error(`${gwName} mailOnError sending failed: ${err.message}`) })
...
function sendMail (config, content) {
  return new Promise((resolve, reject) => {
    if (!config || !config.smtp || !(config.smtp.enabled === true) || isMailLock) return resolve() // not sending mail
    isMailLock = true
    setTimeout(function () { // release lock after "sendInterval" minutes
      isMailLock = false
    }, (config.smtp.sendInterval || 15) * 1000 * 60)
    ...
    let smtpConfig = {
      'host': config.smtp.host, // e.g. smtp.office365.com
      'port': config.smtp.port || 587,
      'proxy': config.smtp.proxy || null,
      'secure': (config.smtp.port === 465), // false on 25/587
      'tls': { 'ciphers': 'TLSv1.2' }
    }
    if (config.smtp.authenticate) {
      smtpConfig.auth = {}
      smtpConfig.auth.user = config.smtp.username
      smtpConfig.auth.pass = config.smtp.password
    }

    let transporter = nodemailer.createTransport(smtpConfig)
    let mailOptions = {
      'from': config.smtp.username, // sender address
      'to': config.smtp.to, // list of receivers - comma separated
      'cc': config.smtp.cc,
    ...
    transporter.sendMail(mailOptions, function (err, info) {
      ...
      else resolve(`${config.smtp.to}${(config.smtp.cc) ? ',' + config.smtp.cc : ''}`)
    })
  })
} // sendMail
