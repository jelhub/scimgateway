...
let ScimGateway = function () {
    ...
    let gwPassword = ScimGateway.prototype.getPassword('scimgateway.password', configFile);
    ...
    let process_env = 'process.env.' //config.port could be set to "process.env.xxx" e.g process.env.PORT then using environment PORT
    if (isNaN(config.port) && config.port.indexOf(process_env) > -1) config.port = process.env[config.port.substring(process_env.length)]
