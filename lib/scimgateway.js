...
let ScimGateway = function () {
  ...
  if (config.scimversion === '2.0' || config.scimversion === 2) scimDef = require('../lib/scimdef-v2')
  ...
  // ServiceProviderConfigs = scimv1, ServiceProviderConfig = scimv2
  ...
  app.get('(|/:baseEntity)(|/scim)/Schemas/Users', (req, res) => {
    let tx = scimDef.Schemas.Resources[0]
    res.send(tx)
    logger.debug(`${gwName} GET = ${req.originalUrl} Response = ${JSON.stringify(tx)}`)
  })

  app.get('(|/:baseEntity)(|/scim)/Schemas/Groups', (req, res) => {
