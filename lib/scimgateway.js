...
const ScimGateway = function () {
  ...
  const replaceUsrGrp = async (ctx) => {
    ...
    try {
      ...
      if (config.scim.usePutSoftSync) { // usePutSoftSync=true keeps eksisting attributes (and groups) that are not included in PUT jsonBody
        const tmpObj = {}
        for (const key in currentObj) { // only include arrays in clearedObj and later on remove those in newObj having operation delete
          if (Array.isArray(currentObj[key])) {
            tmpObj[key] = currentObj[key]
          }
        }
        if (Object.keys(tmpObj).length > 0) {
          clearedObj = clearObjectValues(tmpObj)
        }
      } else {
      ...
      for (const key in newObj) {
        if (Array.isArray(newObj[key])) {
          ...
          if (typeof (newObj[key]) === 'object') {
            for (const keySub in newObj[key]) {
              if (typeof (newObj[key][keySub]) === 'object') {
                for (const keySubSub in newObj[key][keySub]) {
                  if (currentObj[key] && currentObj[key][keySub] && currentObj[key][keySub][keySubSub] === newObj[key][keySub][keySubSub]) {
                    delete newObj[key][keySub][keySubSub]
                  }
                }
                if (Object.keys(newObj[key][keySub]).length === 0) delete newObj[key][keySub]
              } else {
                if (currentObj[key] && currentObj[key][keySub] === newObj[key][keySub]) {
                  delete newObj[key][keySub]
                }
              }
            }
            if (Object.keys(newObj[key]).length === 0) delete newObj[key]
          } else if (newObj[key] === currentObj[key]) delete newObj[key]
        }
      }
  ...
  if (config.stream.subscriber.enabled) {
    const Subscriber = require('../lib/scim-stream').Subscriber
    const subscriber = async (baseEntity, subject) => {
    ...
    if (config.stream.subscriber.entity && Object.keys(config.stream.subscriber.entity.length > 0)) {
      logger.info(`${gwName}[${pluginName}] now starting SCIM Stream subscribers...`)
    } else {
      logger.error(`${gwName}[${pluginName}] subscriber missing entity configuration: scimgateway.stream.subscriber.entity.xxxx`)
    }
    for (const baseEntity in config.stream.subscriber.entity) {
      if (!config.stream.subscriber.entity[baseEntity].nats) {
        logger.error(`${gwName}[${pluginName}] subscriber missing nats configuration: scimgateway.stream.subscriber.entity.${baseEntity}.nats`)
        continue
      }
      if (!config.stream.subscriber.entity[baseEntity].nats.secret) {
        logger.error(`${gwName}[${pluginName}] subscriber missing secret configuration: scimgateway.stream.subscriber.entity.${baseEntity}.nats.secret`)
        continue
      }
      const subject = config.stream.subscriber.entity[baseEntity].nats.subject
      if (!subject) {
        logger.error(`${gwName}[${pluginName}] subscriber missing subject configuration: scimgateway.stream.subscriber.entity.${baseEntity}.nats.subject`)
        continue
      }
      subscriber(baseEntity, subject)
    }
  }
