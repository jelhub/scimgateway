...
const ScimGateway = function () {
  ...
  if (config.scim.skipTypeConvert === true) multiValueTypes = []
    ...
    multiValueTypes = getMultivalueTypes('User') // not icluding 'Group' => 'members' are excluded
    ...
    '/:baseEntity/(|scim/)(Users|Groups|servicePlans)'], async (ctx) => {
      ...
      if (ctx.query.filter) {
        ...
        try {
          ...
          let identifier = decodeURIComponent(ctx.query.op.substring(ctx.query.op.indexOf('"')).replace(/"/g, '')) // bjensen / UserGroup-1
          const arrOp = ctx.query.op.split(' ')
          const getObj = {
            operator: arrOp[1],
            filter: arrOp[0], // e.g. userName
            identifier: identifier // e.g. bjensen
          }
          const data = await this[handle.exploreMethod](ctx.params.baseEntity, getObj, ctx.query.attributes ? ctx.query.attributes : '', ctx.query.startIndex ? parseInt(ctx.query.startIndex) : undefined, count ? parseInt(count) : undefined)
          let scimdata = data

          if (!scimdata || !scimdata.Resources) {
            scimdata = {
              Resources: [],
              totalResults: null
            }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`], async (ctx) => {
    ...
    try {
      ...
      for (const key in data) { // merge any result e.g: data = {'id': 'xxxx'}
      ...
      if (!jsonBody.id) { // retrieve id
        ...
        try {
          if (handle.getMethod === 'getUser') {
            obj = await this[handle.getMethod](ctx.params.baseEntity, { filter: 'userName', identifier: jsonBody.userName }, 'id,userName')
          } else if (handle.getMethod === 'getGroup') {
            obj = await this[handle.getMethod](ctx.params.baseEntity, { filter: 'displayName', identifier: jsonBody.displayName }, 'id,displayName')
          }
        } catch (err) { }
  ...
  `/:baseEntity/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`], async (ctx) => {
    ...
    if (strBody === '{}') {
      ...
      try {
        // get current object
        logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
        const currentObj = await this[handle.getMethod](ctx.params.baseEntity, { filter: 'id', identifier: id }, '')
        ...
        // get updated object
        logger.debug(`${gwName}[${pluginName}] calling "${handle.getMethod}" and awaiting result`)
        scimdata = await this[handle.getMethod](ctx.params.baseEntity, { filter: 'id', identifier: id }, '')

        // include groups
        if (handle.getMethod === handler.users.getMethod) {
          logger.debug(`${gwName}[${pluginName}] calling "${handler.groups.inclusionMethod}" and awaiting result`)
          const grps = await this[handler.groups.inclusionMethod](ctx.params.baseEntity, id, 'members.value,id,displayName')
          if (grps && Array.isArray(grps) && grps.length > 0) {
            scimdata.groups = []
            for (let i = 0; i < grps.length; i++) {
              const el = {}
...
//
const getMultivalueTypes = (objName) => { // objName = 'User' or 'Group'
  if (!objName) return []
...
const convertedScim20 = (obj) => {
  ...
  for (let i = 0; i < o.Operations.length; i++) {
    ...
    if (element.path) {
      ...
      rePattern = new RegExp(/^(.*)\[value eq (.*)\]$/) // "path":"members[value eq \"bjensen\"]"
      arrMatches = element.path.match(rePattern)
