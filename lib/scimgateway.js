...
const ScimGateway = function () {
  ...
  router.get(['/(|scim/)(Users|Groups|servicePlans)',
    ...
    if (ctx.query.filter) {
      ...
      try {
        const data = await this[handle.exploreMethod](ctx.params.baseEntity, ctx.query.attributes ? ctx.query.attributes : '', ctx.query.startIndex ? parseInt(ctx.query.startIndex) : undefined, ctx.query.count ? parseInt(ctx.query.count) : undefined)
        let scimdata = data
        scimdata = addResources(scimdata, ctx.query.startIndex)
        scimdata = addSchemas(scimdata, handle.description, isScimv2)
        ctx.body = scimdata
