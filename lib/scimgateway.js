...
let ScimGateway = function () {
  ...
  handler.servicePlans = { // plugin-azure using "CustomSCIM"
  ...
  app.get('(|/:baseEntity)(|/scim)/(|Users|Groups|servicePlans)/:id', (req, res) => {
  ...
  app.get('(|/:baseEntity)(|/scim)/(|Users|Groups|servicePlans)', (req, res) => {
    ...
    let handle = handler[u]
    if (req.query.attributes === handle.uniqueAttr && !req.query.filter) {
      // ==========================================
      //             EXPLORE
      // ==========================================
      //
      ...
      //
      logger.debug(`${gwName} [Explore ${handle.description}s]`)
        ...
        if ((arrFilter[0] === handle.uniqueAttr || arrFilter[0] === 'id' || arrFilter[0] === 'externalId' || arrFilter[0] === 'members')) {
          ...
          this.emit(handle.inclusionEmitter, req.params.baseEntity, identifier, req.query.attributes, function (err, data) {
            if (err) {
              ...
              if (data) scimdata.Resources = data // array
              scimdata = addPagination(scimdata, req.query.startIndex)
              res.send(scimdata)
              logger.debug(`${gwName}[${pluginName}] GET = ${req.originalUrl} Response = ${JSON.stringify(scimdata)}`)
            }
  ...
  app.post('(|/:baseEntity)(|/scim)/(|Users|Groups)(|.json)(|.xml)', (req, res) => {
  ...
  //
  app.patch('(|/:baseEntity)(|/scim)/(|Users|Groups|servicePlans)/:id', (req, res) => {
    let u = req.originalUrl.substr(0, req.originalUrl.lastIndexOf('/'))
    u = u.substr(u.lastIndexOf('/') + 1) // u = Users, Groups
    let handle = handler[u]
    let id = req.params.id
        ...
        for (let key in dotParseObj) { // {"active": "true"}
          ...
          for (let key2 in mapObj) {
            if (mapObj[key2].mapTo === key) {
              ...
              break
            } else if (mapObj[key2].type === 'array' && key.indexOf('.value') > 0) { // servicePlan.0.value
    ...
    let recursiveStrMap = function (obj, dotPath) { // converts inbound/outbound regarding endpointMap type of attribute
      for (let key in obj) {
        if (Array.isArray(obj[key]) && obj[key].length < 1) {
          delete obj[key] // don't like empty array if expected [{'value': null}]
          continue
        }
        ...
        if (direction === 'outbound') { // outbound
          ...
          if (obj[key] === '') obj[key] = null
          else if (mapObj[dotKey] && mapObj[dotKey].type) {
            ...
            } else if (type === 'array' && !Array.isArray(obj[key])) obj[key] = [obj[key]]
          }
