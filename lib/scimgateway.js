...
const ScimGateway = function () {
  ...
  // build noneCoreSchemaObj containing all none core attributes - getUser must return those on correct format
  const noneCoreSchemaObj = {}
  if (!config.scim.skipCoreMapping || config.scim.skipCoreMapping !== true) {
    for (let i = 0; i < scimDef.Schemas.Resources.length; i++) {
      if (isScimv2) {
        if (scimDef.Schemas.Resources[i].id && !scimDef.Schemas.Resources[i].id.includes(':core:')) {
          if (scimDef.Schemas.Resources[i].attributes) {
            const id = scimDef.Schemas.Resources[i].id
            const attrs = scimDef.Schemas.Resources[i].attributes
            for (let j = 0; j < attrs.length; j++) {
              if (attrs[j].name) {
                noneCoreSchemaObj[attrs[j].name] = id // { "employeeNumber": "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User" }
              }
            }
          }
        }
        ...
        const attrs = scimDef.Schemas.Resources[i].attributes
        for (let j = 0; j < attrs.length; j++) {
          if (attrs[j].schema && !attrs[j].schema.includes(':core:')) {
            noneCoreSchemaObj[attrs[j].name] = attrs[j].schema //  { "employeeNumber": "urn:scim:schemas:extension:enterprise:1.0"}
          }
        }
      }
    }
    ...
    try {
      ...
      if (!data || JSON.stringify(data) === '{}') {
        ...
        for (const key in data) { // exclude null and empty object/array
          ...
          if (noneCoreSchemaObj[key]) { // none core attributes must be schema objects
            if (!data[noneCoreSchemaObj[key]]) {
              data[noneCoreSchemaObj[key]] = {} // { urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {} }
              if (!data.schemas) data.schemas = [noneCoreSchemaObj[key]]
              else data.schemas.push(noneCoreSchemaObj[key])
            }
            if (key === 'manager' && data[key]) {
              if (isScimv2) data[noneCoreSchemaObj[key]][key] = { value: data[key] }
              else data[noneCoreSchemaObj[key]][key] = { managerId: data[key] }
            } else data[noneCoreSchemaObj[key]][key] = data[key]
            delete data[key]
          }
        }
      ...
      if (arrFilter.length > 2 && arrFilter[1] === 'eq') {
        ...
        if (getObj.filter === 'groups.value' || getObj.filter === 'members.value') {
          ...
          try {
            ...
            let scimdata = data
            scimdata = addResources(scimdata, ctx.query.startIndex, noneCoreSchemaObj)
            scimdata = addSchemas(scimdata, handle.description, isScimv2)
            ctx.body = scimdata
            return null
          } catch (err) {
  ...
  router.patch([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    ...
    const strBody = JSON.stringify(jsonBody)
    if (strBody === '{}') {
      let err = new Error('Not accepting empty or none JSON formatted PATCH requests')
      err = jsonErr(config.scim.version, pluginName, ctx.status, err)
      ctx.status = 500
      ctx.body = err
    } else {
      ...
      try {
        ...
        if (ctx.query.attributes || ctx.query.excludedAttributes) {
          ...
          if (ctx.query.excludedAttributes) {
            const arrEx = ctx.query.excludedAttributes.split(',')
            for (let i = 0; i < arrEx.length; i++) {
              delete data[arrEx[i]]
            }
          ...
          delete data.password
          const scimdata = addSchemas(data, handle.description, isScimv2)
          ctx.status = 200
          ctx.body = scimdata
        } else ctx.status = 204
      } catch (err) {
...
ScimGateway.prototype.convertedScim = function convertedScim (obj) {
  ...
  for (const key in scimdata) {
    ...
    } else if (key.startsWith('urn:') && (key.includes(':1.0:') || key.includes(':2.0:'))) { // "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {...}
      if (typeof scimdata[key] === 'object') {
        if (scimdata[key].manager) {
          if (scimdata[key].manager.operation && scimdata[key].manager.operation === 'delete') { // scim v1.1
            scimdata[key].manager = ''
          } else {
            if (scimdata[key].manager.value) scimdata[key].manager = scimdata[key].manager.value
            else if (scimdata[key].manager.managerId) scimdata[key].manager = scimdata[key].manager.managerId // scim v1.1
          }
        }
        ScimGateway.prototype.extendObj(scimdata, scimdata[key])
        delete scimdata[key]
      }
    }
  }
...
const addResources = (data, startIndex, noneCoreSchemaObj) => {
  ...
  for (let i = 0; i < res.Resources.length; i++) {
    for (const key in res.Resources[i]) {
      if (key === 'password') delete res.Resources[i].password
      else if (noneCoreSchemaObj && noneCoreSchemaObj[key]) { // none core attributes must be schema objects
        const isScimv1 = noneCoreSchemaObj[key].includes(':1.0')
        if (!res.Resources[i][noneCoreSchemaObj[key]]) {
          res.Resources[i][noneCoreSchemaObj[key]] = {} // { "urn:scim:schemas:extension:enterprise:1.0": {} }
          if (isScimv1) {
            if (!res.schemas) res.schemas = []
            if (!res.schemas.includes(noneCoreSchemaObj[key])) res.schemas.push(noneCoreSchemaObj[key])
          }
        }
        if (key === 'manager' && res.Resources[i][key]) {
          if (isScimv1) res.Resources[i][noneCoreSchemaObj[key]][key] = { managerId: res.Resources[i][key] }
          else res.Resources[i][noneCoreSchemaObj[key]][key] = { value: res.Resources[i][key] }
        } else res.Resources[i][noneCoreSchemaObj[key]][key] = res.Resources[i][key]
        delete res.Resources[i][key]
      }
    }
  }
...
const addSchemas = (data, obj, isScimv2) => {
  if (!isScimv2) {
    ...
    if (data.Resources || obj === 'User' || obj === 'Group') {
...
const convertedScim20 = (obj) => {
  ...
  o.Operations.forEach(function (element, index) {
    ...
    if (element.path) {
      ...
      if (element.path.startsWith('urn:') && element.path.includes(':2.0:')) { // {"path": "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User:department"}
        const arr = element.path.split(':')
        element.path = arr[arr.length - 1] // department
      }
