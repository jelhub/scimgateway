...
let ScimGateway = function () {
      ...
      if (arrFilter.length > 2 && arrFilter[1] === 'eq') {
        if ((arrFilter[0] === handle.uniqueAttr || arrFilter[0] === 'id' || arrFilter[0] === 'externalId' || arrFilter[0] === 'members')) {
          ...
          try {
            ...
            if ((!data || JSON.stringify(data) === '{}') && !isScimv2) { // user/group not found, scim1.1 => http 404, scim2.0 http 200 and empty resource
  ...
  router.post([`/(|scim/)(!${undefined}|Users|Groups)(|.json)(|.xml)`,
    ...
    let jsonBody = JSON.stringify(ctx.request.body)
    if (jsonBody === '{}') {
      let err = new Error('Not accepting empty or none JSON formatted POST requests')
      ctx.status = 500
      ctx.body = `${gwName}[${pluginName}] ${err.message}`
    } else {
      logger.debug(`${gwName}[${pluginName}] POST ${ctx.originalUrl} Body = ${jsonBody}`)
      jsonBody = JSON.parse(jsonBody) // back to json
      ...
      try {
        ...
        let location = `${ctx.origin}${ctx.path}/${jsonBody.userName ? jsonBody.userName : jsonBody.displayName}`
  ...
  router.patch([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    ...
    if (jsonBody === '{}') {
      ...
      logger.debug(`${gwName}[${pluginName}] PATCH ${ctx.originalUrl} Body = ${jsonBody}`)
  ...
  router.put([`/(|scim/)(!${undefined}|Users|Groups|servicePlans)/:id`,
    ...
    if (jsonBody === '{}') {
      ...
      logger.debug(`${gwName}[${pluginName}] [Modify ${handle.description}] id=${id}`)
      logger.debug(`${gwName}[${pluginName}] PUT ${ctx.originalUrl} Body = ${jsonBody}`)
      jsonBody = JSON.parse(jsonBody)
      let scimdata
      if (isScimv2) scimdata = convertedScim20(jsonBody) // patch/put differences v1/v2
      else scimdata = ScimGateway.prototype.convertedScim(jsonBody)
