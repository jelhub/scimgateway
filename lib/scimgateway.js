...
let ScimGateway = function () {
  ...
  var log = new Log(config.loglevel, path.join(`${logDir}`, `${pluginName}.log`))
  ...
  // all config can be set based on environment variables - except password, secret and token
  // syntax = "process.env.xxx" e.g. config.port could be set to "process.env.PORT", then using environment variable PORT
  let processEnv = 'process.env.'
  if (config.scimversion && config.scimversion.constructor === String && config.scimversion.indexOf(processEnv) > -1) config.scimversion = process.env[config.scimversion.substring(processEnv.length)]
  if (config.localhostonly && config.localhostonly.constructor === String && config.localhostonly.indexOf(processEnv) > -1) config.localhostonly = process.env[config.localhostonly.substring(processEnv.length)]
  if (config.port && config.port.constructor === String && config.port.indexOf(processEnv) > -1) config.port = process.env[config.port.substring(processEnv.length)]
  if (config.loglevel.file && config.loglevel.file.constructor === String && config.loglevel.file.indexOf(processEnv) > -1) config.loglevel.file = process.env[config.loglevel.file.substring(processEnv.length)]
  if (config.loglevel.console && config.loglevel.console.constructor === String && config.loglevel.console.indexOf(processEnv) > -1) config.loglevel.console = process.env[config.loglevel.console.substring(processEnv.length)]
  if (config.auth.basic.username && config.auth.basic.username.constructor === String && config.auth.basic.username.indexOf(processEnv) > -1) config.auth.basic.username = process.env[config.auth.basic.username.substring(processEnv.length)]
  if (config.auth.bearer.jwt.azure.tenantIdGUID && config.auth.bearer.jwt.azure.tenantIdGUID.constructor === String && config.auth.bearer.jwt.azure.tenantIdGUID.indexOf(processEnv) > -1) config.auth.bearer.jwt.azure.tenantIdGUID = process.env[config.auth.bearer.jwt.azure.tenantIdGUID.substring(processEnv.length)]
  if (config.auth.bearer.jwt.standard.publicKey && config.auth.bearer.jwt.standard.publicKey.constructor === String && config.auth.bearer.jwt.standard.publicKey.indexOf(processEnv) > -1) config.auth.bearer.jwt.standard.publicKey = process.env[config.auth.bearer.jwt.standard.publicKey.substring(processEnv.length)]
  if (config.auth.bearer.jwt.standard.options.issuer && config.auth.bearer.jwt.standard.options.issuer.constructor === String && config.auth.bearer.jwt.standard.options.issuer.indexOf(processEnv) > -1) config.auth.bearer.jwt.standard.options.issuer = process.env[config.auth.bearer.jwt.standard.options.issuer.substring(processEnv.length)]
  if (config.certificate.key && config.certificate.key.constructor === String && config.certificate.key.indexOf(processEnv) > -1) config.certificate.key = process.env[config.certificate.key.substring(processEnv.length)]
  if (config.certificate.cert && config.certificate.cert.constructor === String && config.certificate.cert.indexOf(processEnv) > -1) config.certificate.cert = process.env[config.certificate.cert.substring(processEnv.length)]
  if (config.certificate.ca && config.certificate.ca.constructor === String && config.certificate.ca.indexOf(processEnv) > -1) config.certificate.ca = process.env[config.certificate.ca.substring(processEnv.length)]
  if (config.certificate.pfx.bundle && config.certificate.pfx.bundle.constructor === String && config.certificate.pfx.bundle.indexOf(processEnv) > -1) config.certificate.pfx.bundle = process.env[config.certificate.pfx.bundle.substring(processEnv.length)]
  if (config.emailOnError.smtp.enabled && config.emailOnError.smtp.enabled.constructor === String && config.emailOnError.smtp.enabled.indexOf(processEnv) > -1) config.emailOnError.smtp.enabled = process.env[config.emailOnError.smtp.enabled.substring(processEnv.length)]
  if (config.emailOnError.smtp.host && config.emailOnError.smtp.host.constructor === String && config.emailOnError.smtp.host.indexOf(processEnv) > -1) config.emailOnError.smtp.host = process.env[config.emailOnError.smtp.host.substring(processEnv.length)]
  if (config.emailOnError.smtp.port && config.emailOnError.smtp.port.constructor === String && config.emailOnError.smtp.port.indexOf(processEnv) > -1) config.emailOnError.smtp.port = process.env[config.emailOnError.smtp.port.substring(processEnv.length)]
  if (config.emailOnError.smtp.proxy && config.emailOnError.smtp.proxy.constructor === String && config.emailOnError.smtp.proxy.indexOf(processEnv) > -1) config.emailOnError.smtp.proxy = process.env[config.emailOnError.smtp.proxy.substring(processEnv.length)]
  if (config.emailOnError.smtp.authenticate && config.emailOnError.smtp.authenticate.constructor === String && config.emailOnError.smtp.authenticate.indexOf(processEnv) > -1) config.emailOnError.smtp.authenticate = process.env[config.emailOnError.smtp.authenticate.substring(processEnv.length)]
  if (config.emailOnError.smtp.username && config.emailOnError.smtp.username.constructor === String && config.emailOnError.smtp.username.indexOf(processEnv) > -1) config.emailOnError.smtp.username = process.env[config.emailOnError.smtp.username.substring(processEnv.length)]
  if (config.emailOnError.smtp.sendInterval && config.emailOnError.smtp.sendInterval.constructor === String && config.emailOnError.smtp.sendInterval.indexOf(processEnv) > -1) config.emailOnError.smtp.sendInterval = process.env[config.emailOnError.smtp.sendInterval.substring(processEnv.length)]
  if (config.emailOnError.smtp.to && config.emailOnError.smtp.to.constructor === String && config.emailOnError.smtp.to.indexOf(processEnv) > -1) config.emailOnError.smtp.to = process.env[config.emailOnError.smtp.to.substring(processEnv.length)]
  if (config.emailOnError.smtp.cc && config.emailOnError.smtp.cc.constructor === String && config.emailOnError.smtp.cc.indexOf(processEnv) > -1) config.emailOnError.smtp.cc = process.env[config.emailOnError.smtp.cc.substring(processEnv.length)]
  ...
  app.use(morgan('combined', { 'stream': log.stream }))   // express logging to log.stream (combined/common) instead of: app.use(express.logger('dev'));  /* 'default', 'short', 'tiny', 'dev' */
  ...
  app.use(function (req, res, next) { // authentication/authorization
    async.parallel(
      [
        function (callback) { // ping - no auth
          if (req.url === '/ping') return callback(null, true)
      ...
      function (err, results) { // final callback
        ...
        if (!isAuthenticated && !err) {
          if (!req.token) err = new Error(`${req.url} request without authentication information`)
          else {
            err = new Error(`${req.url} request with unsupported authorization bearer or missing plugin configuration`)
          ...
          if (req.url !== '/favicon.ico') logger.error(`${gwName} ${err.message}`)
        } else if (!isAuthenticated || err) {
          res.setHeader('WWW-Authenticate', 'Basic realm=""')
          if (pwErrCount < 3) {
            pwErrCount += 1
            res.status(401).end('Access denied')
            logger.error(`${gwName} ${req.url} ${err.message}`)
          } else { // delay brute force attempts
            setTimeout(function () {
              res.status(401).end('Access denied')
            }, 1000 * 60 * 2)
            logger.error(`${gwName} ${req.url} ${err.message} => delaying response with 2 minutes to prevent brute force`)
          }
        } else {
          res.setHeader('Content-Type', 'application/json; charset=utf-8') // IE don't support 'application/json', neither IE nor Edge supports 'application/scim+json'
          return next()
        }
      }
    ) // async
  ...
  app.get('/ping', (req, res) => { // auth not required
    let tx = 'hello'
    res.setHeader('Content-Type', 'text/plain; charset=utf-8')
    res.send(tx)
    logger.debug(`${gwName} GET = ${req.originalUrl} Response = ${tx}`)
  })
  ...
  log.on('emailOnError', function (msg, callback) { // sending mail on error
    callback(null) // releasing log emitter and continue
    if (!config.emailOnError || !config.emailOnError.smtp || !(config.emailOnError.smtp.enabled === true) || isMailLock) return // not sending mail
    isMailLock = true
    ...
    transporter.sendMail(mailOptions, function (err, info) {
      if (err) logger.error(`${gwName} mailOnError sending failed: ${err.message}`)
      else logger.debug(`${gwName} mailOnError sent to: ${config.emailOnError.smtp.to}${(config.emailOnError.smtp.cc) ? ',' + config.emailOnError.smtp.cc : ''}`)
    })
  }) // emailOnError
