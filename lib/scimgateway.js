      ...
      function (err, results) { // final callback
        ...
        if (!isAuthenticated && !err) {
          ...
          if (pwErrCount < 3) {
            pwErrCount += 1
            res.status(401).end('Access denied')
            logger.error(`${gwName} ${req.url} request not authorized`)
          } else { // delay brute force attempts
            setTimeout(function () {
              res.status(401).end('Access denied')
            }, 1000 * 60 * 2)
            logger.error(`${gwName} ${req.url} request not authorized => delaying response with 2 minutes to prevent brute force`)
          }
        } else {
