...
let ScimGateway = function () {
  ...
  var log = new Log({ 'category': pluginName, 'file': 'debug', 'console': 'debug', 'colorize': process.stdout.isTTY || false }, path.join(`${logDir}`, `${pluginName}.log`))
  ...
  if (config.scim.customSchema) { // merge plugin custom schema extension into core schemas
    ...
    let schemas = ['User', 'Group']
    let customMerged = false
    for (let i = 0; i < schemas.length; i++) {
      let schema = scimDef.Schemas.Resources.find(el => el.name === schemas[i])
      let customSchema = custom.find(el => el.name === schemas[i])
      if (schema && customSchema && Array.isArray(customSchema.attributes)) {
        let arr1 = schema.attributes // core:1.0/2.0 schema
        let arr2 = customSchema.attributes
    ...
    if (!customMerged) {
      throw new Error(`No custom SCIM schema attributes have been merged. Make sure using correct format e.g. \
                      [{"name": "User", "attributes" : [...]}]. \
                      Also make sure attribute names in attributes array do not conflict with core:1.0/2.0 SCIM attribute names`.replace(/ {2}/g, ''))
    }
  }
  ...
  let logResult = async (ctx, next) => {
    ...
    if (ctx.request.url !== '/favicon.ico') {
      if (ctx.response.status < 200 || ctx.response.status > 299) {
        logger.error(`${gwName}[${pluginName}] ${ellapsed} ${ctx.request.ip} ${userName} ${ctx.request.method} ${ctx.request.href} Body = ${JSON.stringify(ctx.request.body)} Response = ${JSON.stringify(res)}${(config.loglevel.file === 'debug' && ctx.request.url !== '/ping') ? '\n' : ''}`)
      } else logger.info(`${gwName}[${pluginName}] ${ellapsed} ${ctx.request.ip} ${userName} ${ctx.request.method} ${ctx.request.href} Body = ${JSON.stringify(ctx.request.body)} Response = ${JSON.stringify(res)}${(config.loglevel.file === 'debug' && ctx.request.url !== '/ping') ? '\n' : ''}`)
