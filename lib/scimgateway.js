...
ScimGateway.prototype.convertedScim = function convertedScim (obj) {
  ...
  for (const key in scimdata) {
    if (Array.isArray(scimdata[key]) && (scimdata[key].length > 0)) {
      if (scimdata[key][0].value) {
        scimdata[key][0].value = decodeURIComponent(scimdata[key][0].value)
      }
      if (scimdata[key][0].type) { // exclude "none type" multivalue attributes (e.g groups and x509Certificates)
          ...
          if (element.value) {
            if (Array.isArray(element.value)) {
              element.value.forEach(function (el) {
                groupMembers.push({ operation: 'delete', value: decodeURIComponent(el.value) })
              })
            } else if (element.value.value) groupMembers.push({ operation: 'delete', value: decodeURIComponent(element.value.value) })
