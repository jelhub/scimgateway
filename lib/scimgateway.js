...
ScimGateway.prototype.convertedScim = function convertedScim (obj) {
  ...
  for (const key in scimdata) {
    if (Array.isArray(scimdata[key]) && (scimdata[key].length > 0)) {
    ...
    } else if (key.startsWith('urn:')) { // "urn:ietf:params:scim:schemas:extension:enterprise:2.0:User": {...}
      if (scimdata[key].manager) {
        if (scimdata[key].manager.operation && scimdata[key].manager.operation === 'delete') { // scim v1.1
          ...
          if (scimdata[key].manager.value) { // scim v2.0
            const val = scimdata[key].manager.value
            delete scimdata[key].manager
            scimdata[key].manager = val
          } else if (scimdata[key].manager.managerId) { // scim v1.1
            const val = scimdata[key].manager.managerId
            delete scimdata[key].manager
            scimdata[key].manager = val
          }
        }
...
// TO-DO: enterprise and custom schemas attribute handling (dot in key problem)
  ...
  if (data.Resources) {
    ...
    for (let i = 0; i < data.Resources.length; i++) {
      for (const key in data.Resources[i]) {
        if (key === 'password') delete data.Resources[i].password
        if (key.startsWith('urn:')) {
          if (key.includes(':1.0')) {
            if (!data.schemas) data.schemas = []
            if (!data.schemas.includes(key)) data.schemas.push(key)
          }
          if (data.Resources[i][key].manager && typeof data.Resources[i][key].manager !== 'object') {
            const val = data.Resources[i][key].manager
            delete data.Resources[i][key].manager
            if (key.includes(':1.0')) data.Resources[i][key].manager = { managerId: val }
            else data.Resources[i][key].manager = { value: val }
          }
        }
      }
    }
    ...
    for (const key in data) {
      if (key === 'password') delete data.password
      else if (key.startsWith('urn:')) { // add none core schema e.g. urn:ietf:params:scim:schemas:extension:enterprise:2.0:User
        if (!data.schemas) data.schemas = [key]
        else if (!data.schemas.includes(key)) data.schemas.push(key)
        if (data[key].manager && typeof data[key].manager !== 'object') {
          const val = data[key].manager
          delete data[key].manager
          if (key.includes(':1.0')) data[key].manager = { managerId: val }
          else data[key].manager = { value: val }
        }
      }
    }
  }
          ...
          if (ScimGateway.prototype.isMultivalue('User', pathRoot)) {
            ...
            if (Array.isArray(element.value)) dot.str(element.path, element.value[0].value, scimdata)
              ...
              if (element.path.startsWith('urn:')) { // can't use dot.str on key having dot e.g. urn:ietf:params:scim:schemas:extension:enterprise:2.0:User:department
                let rootKey, key
                let s = ':User:'
                let pos = element.path.indexOf(s)
                if (pos < 0) {
                  s = ':User'
                  pos = element.path.indexOf(s)
                }
                if (pos < 0) {
                  s = ':Group:'
                  pos = element.path.indexOf(s)
                ...
                if (pos > 0 && (s === ':User:' || s === ':Group:')) {
                  rootKey = element.path.substring(0, pos + s.length - 1) // urn:ietf:params:scim:schemas:extension:enterprise:2.0:User
                  key = element.path.substring(pos + s.length) // department
                  if (!scimdata[rootKey]) scimdata[rootKey] = {}
                  dot.str(key, value, scimdata[rootKey])
                } else if (pos > 0 && (s === ':User' || s === ':Group')) { // value is object
                  rootKey = element.path.substring(0, pos + s.length) // urn:ietf:params:scim:schemas:extension:enterprise:2.0:User
                  if (!scimdata[rootKey]) scimdata[rootKey] = {}
                  for (const key in value) {
                    if (key === 'manager') {
                      if (value.manager.value) scimdata[rootKey].manager = value.manager.value
                      else if (value.manager.managerId) scimdata[rootKey].manager = value.manager.managerId
                    } else scimdata[rootKey][key] = value[key]
                  }
                }
        ...
        if (element.path === 'members' || element.path === 'groups' || isRoles) { // members => Group attribute, groups => User attribute
          ...
          if (ScimGateway.prototype.isMultivalue('User', pathRoot)) {
            ...
            if (p.startsWith('urn:')) { // can't use dot.str on key having dot e.g. urn:ietf:params:scim:schemas:extension:enterprise:2.0:User:department
              ...
              if (pos > 0) {
                rootKey = p.substring(0, pos + s.length - 1) // urn:ietf:params:scim:schemas:extension:enterprise:2.0:User
                key = p.substring(pos + s.length) // department
              }
              if (!scimdata[rootKey]) scimdata[rootKey] = {}
