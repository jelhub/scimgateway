...
var ScimGateway = function () {
    ...
    app.get('(|/:baseEntity)(|/scim)/Users/:id', (req, res) => {
        ...
        this.emit('getUser', req.params.baseEntity, id, null, function (err, data) {
            if (err) {
                ...
                data.schemas = (new scimDef.scimResource()).schemas;
    ...
    app.get('(|/:baseEntity)(|/scim)/Groups/:id', (req, res) => {
        ...
        this.emit('getGroup', req.params.baseEntity, id, null, function (err, data) {
            if (err) {
                ...
                data.schemas = scimDef.ServiceProviderConfigs.schemas;
                if (data.meta) data.meta.location = location;
                else {
                    data.meta = {};
                    data.meta.location = location;
    ...
    app.delete('(|/:baseEntity)(|/scim)/Users/:id', (req, res) => {
        ...
        this.emit('deleteUser', req.params.baseEntity, id, function (err) {
            if (err) {
                ...
                res.status(204).send();
                logger.debug(`${gwName}[${pluginName}] PATCH = ${req.originalUrl} Response = 204 (no content)`);
            }
        });
    ...
    app.patch('(|/:baseEntity)(|/scim)/Users/:id', (req, res) => {
        ...
        req.on('end', () => {
            ...
            if (scimdata.Operations) scimdata = convertPatch11(scimDef, 'User', scimdata); // scim version 2.0 => convert to 1.1 standard
    ...
    app.post('(|/:baseEntity)(|/scim)/Groups(|.json)(|.xml)', (req, res) => {
        ...
        req.on('end', () => {
            ...
            if (groupObj === null) {
                ...
                var groupObj = JSON.parse(strBody);
                ...
                this.emit('createGroup', req.params.baseEntity, groupObj, function (err) {
                    if (err) {
                        ...
                        let protocol = 'http';
                        if (req.socket._tlsOptions) protocol = 'https';
                        let location = `${protocol}://${req.headers.host}${req.originalUrl}/${groupObj.displayName}`;
                        let scimdata =
                            {
                                "meta": {
                                    "location": location
                                },
                                "id": groupObj.displayName
                            }
                        res.setHeader('Location', `${location}`);
                        res.status(201).send(scimdata);
                        logger.debug(`${gwName}[${pluginName}] POST = ${req.originalUrl} Response = ${JSON.stringify(scimdata)}`);
    ...
    app.patch('(|/:baseEntity)(|/scim)/Groups/:id', (req, res) => {
        ...
        req.on('end', () => {
            logger.debug(`${gwName} PATCH = ${req.originalUrl} Body = ${strBody}`);
            var scimdata = JSON.parse(strBody);
            if (scimdata.Operations) scimdata = convertPatch11(scimDef, 'Group', scimdata); // scim version 2.0 => convert to 1.1 standard
...
//
// Convert SCMIM 2.0 patch to SCIM 1.1 standard
//
// Scim 2.0:
...
//   meta includes cleared/deleted single value attributes, exampel: { meta: { attributes: [ 'name.givenName', 'title' ] } }
//   Multivalue attributes includes scim2.0 array with operation value (modify/create/delete)
//
var convertPatch11 = function convertPatch11(scimDef, objName, data) {
    let scimdata = {};
    let groupMembers = [];
    ...
    data.Operations.forEach(function (element, index) {
        ...
        let arrMatches = element.path.match(rePattern);
        ...
        if (element.path) element.path = element.path.replace(/\[.*/g, '') // "path":"addresses[type eq \"work\"].value" removes [xxx]yy
        if ((element.op).toLowerCase() === 'replace') {
            if (isMultivalue(scimDef, 'User', element.path)) {
                if (!scimdata[element.path]) scimdata[element.path] = [];
                element.value.operation = 'modify' // new operation
                if (type) element.value[0].type = type;
                scimdata[element.path].push(element.value);
            }
            else dot.str(element.path, element.value[0].value, scimdata); // handle e.g name.familyName
        }
        else if ((element.op).toLowerCase() === 'add') {
            if (element.path === 'members') {  //Group
                element.value.forEach(function (el) {
                    groupMembers.push({ "value": el.value });
                });
            }
            else { // User
                if (isMultivalue(scimDef, 'User', element.path)) {
                    if (!scimdata[element.path]) scimdata[element.path] = [];
                    element.value.operation = 'create' // new operation
                    if (type) element.value[0].type = type;
                    scimdata[element.path].push(element.value);
                }
                else dot.str(element.path, element.value[0].value, scimdata);
            }
        }
            ...
            if (element.path === 'members') {  //Group
                ...
                if (isMultivalue(scimDef, 'User', element.path)) {
                    if (!scimdata[element.path]) scimdata[element.path] = [];
                    element.value.operation = 'delete' // new operation
                    if (type) element.value[0].type = type;
                    scimdata[element.path].push(element.value);
                }
                else {
                    if (!scimdata.meta) scimdata.meta = { attributes: [] };
                    scimdata.meta.attributes.push(element.path);
                }
            }
        }
...
//
// objName = User or Group
// attr    = attribute to check if multivalue
//
var isMultivalue = function isMultiValue(scimDef, objName, attr) {
    var ret = false;
