...
let ScimGateway = function () {
  ...
  var log = new Log({ 'file': 'debug', 'console': 'debug', 'colorize': true }, path.join(`${logDir}`, `${pluginName}.log`))
  ...
  try {
    config = ScimGateway.prototype.processExtConfig(config, true) // external config support process.env and process.file
  } catch (err) {
    logger.error(`${gwName} ${err.message}`)
    logger.error(`${gwName} stopping...`)
    console.log() // process.exit(1) // may miss unflushed logger updates to logfile
  ...
  if (config.loglevel.hasOwnProperty('colorize') && config.loglevel.colorize === false) logger.transports.console.colorize = false
  ...
  config.loglevel.console && arrValidLevel.includes(config.loglevel.console.toLowerCase()) ? logger.transports.console.level = config.loglevel.console : logger.transports.console.level = 'debug'
  config.loglevel.file && arrValidLevel.includes(config.loglevel.file.toLowerCase()) ? logger.transports.file.level = config.loglevel.file : logger.transports.file.level = 'debug'
...
ScimGateway.prototype.processExtConfig = function processExtConfig (config, nonePlugin) {
    ...
    if (value && value.constructor === String && value.includes(processEnv)) {
      ...
      try {
        ...
        try {
          ...
          let newKey = null
          if (nonePlugin) newKey = `scimgateway.${key}`
          else newKey = `endpoint.${key}`
