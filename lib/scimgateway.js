    ...
    if (element.path) {
        ...
        if (element.path === 'members' || element.path === 'groups' || isRoles) { // members => Group attribute, groups => User attribute
          ...
          if (ScimGateway.prototype.isMultivalue('User', pathRoot)) {
            ...
            if (Array.isArray(element.value)) dot.str(element.path, element.value[0].value, scimdata)
              ...
              if (element.path === 'active' && typeof element.value === 'string') {
                const lcase = element.value.toLowerCase()
                if (lcase === 'true') element.value = true
                else if (lcase === 'false') element.value = false
              }
              dot.str(element.path, element.value, scimdata)
            }
          }
        }
      ...
      for (const key in element.value) {
        if (Array.isArray(element.value[key])) {
          ...
          if (key === 'active' && typeof element.value[key] === 'string') {
            const lcase = element.value[key].toLowerCase()
            if (lcase === 'true') element.value[key] = true
            else if (lcase === 'false') element.value[key] = false
          }
