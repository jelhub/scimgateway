...
var ScimGateway = function () {
    ...
    if (config.oauth) oauthAccessToken = ScimGateway.prototype.getPassword('scimgateway.oauth.accesstoken', configFile);
    ...
    app.get('(|/:baseEntity)(|/scim)/Users/:id', (req, res) => {
        ...
        this.emit('getUser', req.params.baseEntity, id, null, function (err, data) {
            if (err) {
                ...
                let errJson = null;
                if (config.scimversion !== "2.0") { // v1.1
                    errJson =
                        {
                            "Errors": [
                                {
                                    "description": `ScimGateway ${gwName} ${err.message}`,
                                    "code": "404"
                                }
                            ]
                        }
                    ...
                    errJson =
                        {
                            ...
                            "detail": `ScimGateway ${gwName} ${err.message}`,
                            "status": "404"
                        }
                }
                res.status(404).send(errJson);
                logger.error(`${gwName} GET = ${req.originalUrl} Response = ${JSON.stringify(errJson)}`);
            }
            else {
                let protocol = 'http';
