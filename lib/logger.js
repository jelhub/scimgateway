...
let Log = function (loglevel, logfile, _this) { // { loglevel: { file: "debug", console: "debug" } }
  // winston.emitErrs = true // deprecated in 3.0.0
  ...
  const filterSecret = winston.format((info, opts) => {
    ...
    if (info.level === 'error') { // catched by log.on('emailOnError', funcHandler)  for further message handling - e.g sending email
  ...
  const customFormat = winston.format.combine(
    ...
    winston.format.printf(info => {
      const d = new Date()
      const timestamp = `${d.toLocaleDateString()} ${d.toLocaleTimeString()}.${d.getMilliseconds()}`
      return `${timestamp} ${info.level}: ${info.message}`
    })
  )

  const customFormatColorize = winston.format.combine(
    filterSecret(),
  ...
  Log.prototype.logger = winston.createLogger({
    ...
    transports: [
      ...
      new winston.transports.Console({
        ...
        handleExceptions: true,
        stderrLevels: ['error'],
        format: (loglevel.colorize) ? customFormatColorize : customFormat
      })
    ],
    exitOnError: false
  })
