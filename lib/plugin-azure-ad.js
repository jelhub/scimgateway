  ...
  try {
    ...
    for (let i = 0; i < response.body.value.length; ++i) {
      if (response.body.value[i].id && response.body.value[i].userPrincipalName) {
        ...
        const scimUser = { // userName and id is mandatory
...
scimgateway.getUser = async (baseEntity, userName, attributes) => {
  ...
  const user = () => {
    return new Promise(async (resolve, reject) => {
      ...
      const method = 'GET'
      const path = `/users/${querystring.escape(userName)}?$select=${parsedAttr}` // #EXT# need url encoding e.g myaccount_outlook.com#EXT#@mycompany.onmicrosoft.com
      const body = null
      try {
        const response = await doRequest(baseEntity, method, path, body)
        const userObj = response.body
        if (!userObj) {
          const err = new Error('Got empty response when retrieving data for ' + userName)
          return reject(err)
        }
        resolve(userObj)
      } catch (err) {
  ...
  const manager = () => {
    return new Promise(async (resolve, reject) => {
      ...
      const method = 'GET'
      const path = `/users/${querystring.escape(userName)}/manager?$select=id`
      const body = null
      try {
        const response = await doRequest(baseEntity, method, path, body)
        if (!response.body.id) {
          const err = new Error('Manager id not found when retrieving manager for ' + userName)
          return reject(err)
        } else resolve({ manager: { managerId: response.body.id } })
      } catch (err) {
        let statusCode
        try { statusCode = JSON.parse(err.message).statusCode } catch (e) {}
        if (statusCode === 404) return resolve(null) // user have no manager (logged as error by doRequest)
        return reject(err)
      }
    })
  }
  ...
  const license = () => {
    return new Promise(async (resolve, reject) => {
      ...
      const method = 'GET'
      const path = `/users/${querystring.escape(userName)}/licenseDetails`
      const body = null
      ...
      try {
        const response = await doRequest(baseEntity, method, path, body)
        if (!response.body.value) {
          const err = new Error('No content for license information ' + userName)
          return reject(err)
        } else {
  ...
  try {
    ...
    const retObj = await new Promise(async (resolve, reject) => {
      ...
      if (rePattern.test(displayName)) { // using id and not displayName
        ...
        const method = 'GET'
        const path = `/groups/${id}?$select=${parsedAttr}`
        const body = null
        ...
        if (!response.body && !response.body.displayName) {
          ...
          const retObj = scimgateway.endpointMapper('inbound', response.body, scimgateway.endpointMap.microsoftGraphGroup) // endpoint => SCIM/CustomSCIM attribute standard
          return resolve(retObj)
        }
      } else { // using displayName and not id
        const method = 'GET'
        const path = `/groups?$filter=displayName eq '${displayName}'&$select=${parsedAttr}${includeMembers ? ',id' : ''}` // group id needed for includeMembers (retObj.id)
        const body = null
...
scimgateway.getServicePlan = async (baseEntity, servicePlanName, attributes) => {
  ...
  const arrOutbound = (scimgateway.endpointMapper('outbound', attributes, scimgateway.endpointMap.microsoftGraphLicenseDetails)).split(',')
  const arrInbound = (scimgateway.endpointMapper('inbound', attributes, scimgateway.endpointMap.microsoftGraphLicenseDetails)).split(',')
  const method = 'GET'
  ...
  try {
    ...
    const arr = servicePlanName.split('::')
