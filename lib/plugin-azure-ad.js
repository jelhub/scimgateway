...
//          Application permission having "Windows Azure Active Directory" - all "Application Permissions"
//          For password management Application must be member of "User Account Administrator" (powershell import-Module MSOnline)
//
// Notes: For CA Provisioning - import new endpoint using ConnectorXpress using
//        node_modules\scimgateway\resources\Azure - ScimGateway.xml
//
//        Using "Custom SCIM" attributes defined in scimgateway.endpointMap
//        Some functionality will also work using standard SCIM
//        You could use your own version of endpointMap
// =================================================================================
...
scimgateway.on('exploreUsers', function (baseEntity, startIndex, count, callback) {
  ...
  getServiceClient(baseEntity, function (err, serviceClient) {
    ...
    let method = 'get'
    let req = '/users?$select=id,userPrincipalName'
    request[method](graphv1 + req, serviceClient.options, function (err, response, body) {
      ...
      for (let index = startIndex - 1; index < body.value.length && (index + 1 - startIndex) < count; ++index) {
        if (body.value[index].id && body.value[index].userPrincipalName) {
          let upn = body.value[index].userPrincipalName // upn external users: myaccount_outlook.com#EXT#@mycompany.onmicrosoft.com
          let scimUser = { // userName and id is mandatory
            'userName': upn || body.value[index].userPrincipalName,
            'id': body.value[index].id,
            'externalId': upn || body.value[index].userPrincipalName
          }
          ret.Resources.push(scimUser)
        }
      }
      // To-do implement paging using $top query parameter
...
scimgateway.on('exploreGroups', function (baseEntity, startIndex, count, callback) {
  ...
  getServiceClient(baseEntity, function (err, serviceClient) {
    if (err) return callback(err)
    // req = '/directoryRoles' // includes e.g. 'Company Administrator', 'User Account Administrator'
    let method = 'get'
    let req = '/groups'
    request[method](graphv1 + req, serviceClient.options, function (err, response, body) {
      scimgateway.logger.debug(`${pluginName}[${baseEntity}] ${action} ${method.toUpperCase()} =  ${graphv1}${req} Response = ${JSON.stringify(body)}`)
      ...
      for (let index = startIndex - 1; index < body.value.length && (index + 1 - startIndex) < count; ++index) {
        if (body.value[index].id && body.value[index].displayName) {
          let scimGroup = { // displayName and id is mandatory, note: we set id=displayName
            'displayName': body.value[index].displayName,
            'id': body.value[index].id,
            'externalId': body.value[index].displayName
          }
        ...
        function (callback) { // return licenses
          ...
          request[method](graphv1 + req, serviceClient.options, function (err, response, body) {
            ...
            if (!body.value) {
              ...
              if (body.value.length < 1) return callback(null, retObj) // User with no licenses
          ...
          request[method](graphv1 + req, serviceClient.options, function (err, response, body) { // build curretLic
            ...
            request[method](graphv1 + req, serviceClient.options, function (err, response, body) { // build availableLic
              ...
              for (let key in disabledPlans) {
                lic.addLicenses.push({'skuId': key, 'disabledPlans': disabledPlans[key]})
              }
