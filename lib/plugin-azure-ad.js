...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  ...
  if (!path) throw new Error('mandatory if-else logic not fully implemented')
  ...
  try {
    ...
    if (!response.body.value) {
      const err = new Error(`${action}: invalid response: ${JSON.stringify(response)}`)
      throw (err)
    }
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  try {
    ...
    if (newErr.message.includes('userPrincipalName already exists')) newErr.name = 'uniqueness' // maps to scimType error handling
    throw (newErr)
  }
}
...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  if (arrAttr.includes('members.value') || arrAttr.includes('members')) {
    includeMembers = true
    if (!arrAttr.includes('id')) attributes += ',id'
  }
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'displayName', 'externalId'].includes(getObj.attribute)) {
      // mandatory - unique filtering - single unique user to be returned - correspond to getUser() in versions < 4.x.x
      if (getObj.attribute === 'id') path = `/groups/${getObj.value}?$select=${parsedAttr}`
      else path = `/groups?$filter=${getObj.attribute} eq '${getObj.value}'&$select=${parsedAttr}`
...
scimgateway.deleteGroup = async (baseEntity, id) => {
  ...
  scimgateway.logger.debug(`${pluginName}[${baseEntity}] handling "${action}" id=${id}`)
  const err = new Error(`${action} is not supported by ${pluginName}`)
  throw (err)
}
...
scimgateway.getServicePlans = async (baseEntity, getObj, attributes) => {
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'servicePlanName'].includes(getObj.attribute)) {
      ...
      throw new Error(`servicePlans member of user filtering not supported: ${getObj.rawFilter}`)
    } else {
      // optional - simpel filtering
      throw new Error(`simpel filtering not supported: ${getObj.rawFilter}`)
    }
...
const getUser = async (baseEntity, uid, attributes) => { // uid = id, userName (upn) or externalId (upn)
  ...
  const user = () => {
    return new Promise(async (resolve, reject) => {
      ...
      const method = 'GET'
      const path = `/users/${querystring.escape(uid)}?$select=${parsedAttr}` // #EXT# need url encoding e.g myaccount_outlook.com#EXT#@mycompany.onmicrosoft.com
      const body = null
      try {
        const response = await doRequest(baseEntity, method, path, body)
