...
scimgateway.getGroupMembers = async (baseEntity, id, attributes) => {
  ...
  const arrRet = []
  const method = 'GET'
  // const path = `/users/${id}/memberOf?$select=displayName` // includes all e.g. microsoft.graph.group and microsoft.graph.directoryRole
  const path = `/users/${id}/memberOf/microsoft.graph.group?$select=displayName`
  const body = null
