  ...
  getServiceClient(baseEntity, function (err, serviceClient) {
    ...
    async.parallel(
      ...
      function (err, results) { // final callback return completed userObj
        if (err) {
          if (err.message.includes('empty response')) return callback(null, null) // no user found
  ...
  getServiceClient(baseEntity, function (err, serviceClient) {
    ...
    request[method](graphv1 + req, options, function (err, response, body) {
      ...
      if (attrObj.servicePlan) {
        let attrObjTxt = JSON.stringify(attrObj)
        scimgateway.emit('modifyUser', baseEntity, userObj.userName, attrObj, function (err) {
          if (err) {
            scimgateway.logger.error(`${pluginName}[${baseEntity}] modifyUser ${userObj.userName} ${attrObjTxt} ${err}`)
          }
          return callback(null) // not returning err (user have been created)
        })
      } else callback(null)
    })
  })
          ...
          request[method](graphv1 + req, serviceClient.options, function (err, response, body) { // build curretLic
            ...
            if (body.value.length > 0) {
              for (let i = 0; i < body.value.length; i++) { // currentLic = {skuId: [servicePlanId]}
                ...
                for (let index = 0; index < body.value[i].servicePlans.length; index++) {
                  if (body.value[i].servicePlans[index].servicePlanName &&
                    (body.value[i].servicePlans[index].provisioningStatus === 'Success' || body.value[i].servicePlans[index].provisioningStatus === 'PendingInput')) {
  ...
  getServiceClient(baseEntity, function (err, serviceClient) {
    ...
    async.waterfall(
      ...
      function (err, retObj) { // final callback function
        if (err) {
          if (err.message.includes('empty or invalid response')) return callback(null, null) // no group found
          else return callback(err)
        }
        callback(null, retObj)
      }
