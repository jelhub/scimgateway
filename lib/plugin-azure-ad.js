...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  ...
  try {
    ...
    for (let i = 0; i < response.body.value.length; ++i) { // map to corresponding inbound
      const [scimObj] = scimgateway.endpointMapper('inbound', response.body.value[i], scimgateway.endpointMap.microsoftGraphUser) // endpoint => SCIM/CustomSCIM attribute standard
...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  const [attrs] = scimgateway.endpointMapper('outbound', attributes, scimgateway.endpointMap.microsoftGraphGroup) // SCIM/CustomSCIM => endpoint attribute standard
  const method = 'GET'
  const body = null
  let path
  ...
  if (getObj.operator) {
    if (getObj.operator === 'eq' && ['id', 'displayName', 'externalId'].includes(getObj.attribute)) {
      // mandatory - unique filtering - single unique user to be returned - correspond to getUser() in versions < 4.x.x
      if (getObj.attribute === 'id') path = `/groups/${getObj.value}?$select=${attrs.join()}`
    ...
    if (getObj.startIndex && getObj.startIndex > 1) { // paging
      ...
      path = `/groups?$top=${(!getObj.count || getObj.count > 999) ? 999 : getObj.count}` // paging not supported using filter (Azure default page=100, max=999)
    }
  }
  // mandatory if-else logic - end
  ...
  try {
    ...
    for (let i = 0; i < response.body.value.length; ++i) {
      const [scimObj] = scimgateway.endpointMapper('inbound', response.body.value[i], scimgateway.endpointMap.microsoftGraphGroup) // endpoint => SCIM/CustomSCIM attribute standard
      if (scimObj && typeof scimObj === 'object' && Object.keys(scimObj).length > 0) {
        if (includeMembers) {
          const method = 'GET'
          let path = `/groups/${scimObj.id}/members?$select=id,userPrincipalName`
          const body = null
          scimObj.members = []
          do {
            ...
            if (!response.body.value && !Array.isArray(response.body.value)) {
              ...
              response.body.value.forEach(function (el) {
                scimObj.members.push({ value: el.id, display: el.userPrincipalName })
              })
            }
            path = null
            if (response.body['@odata.nextLink']) path = `/groups/${scimObj.id}/members?` + response.body['@odata.nextLink'].split('?')[1] // paging keep search query
          } while (path) // paging
        }
...
scimgateway.getServicePlans = async (baseEntity, getObj, attributes) => {
  ...
  try {
    ...
    if (path === 'getServicePlan') { // special
      ...
      const [arrOutbound] = (scimgateway.endpointMapper('outbound', attributes, scimgateway.endpointMap.microsoftGraphLicenseDetails))
...
const getUser = async (baseEntity, uid, attributes) => { // uid = id, userName (upn) or externalId (upn)
  ...
  const user = () => {
    return new Promise((resolve, reject) => {
      (async () => {
        const [attrs] = scimgateway.endpointMapper('outbound', attributes, scimgateway.endpointMap.microsoftGraphUser) // SCIM/CustomSCIM => endpoint attribute standard
        const method = 'GET'
        const path = `/users/${querystring.escape(uid)}?$select=${attrs.join()}` // #EXT# need url encoding e.g myaccount_outlook.com#EXT#@mycompany.onmicrosoft.com
        const body = null
        try {
          const response = await doRequest(baseEntity, method, path, body)
  ...
  const manager = () => {
    return new Promise((resolve, reject) => {
      (async () => {
        if (!attributes.includes('manager.managerId')) return resolve(null) // request without manager
        const method = 'GET'
        const path = `/users/${querystring.escape(uid)}/manager?$select=id`
        const body = null
        try {
          const response = await doRequest(baseEntity, method, path, body)
          if (!response.body.id) {
            const err = new Error('Manager id not found when retrieving manager for ' + uid)
            return reject(err)
