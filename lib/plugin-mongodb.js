  ...
  for (let el of attrObj.members) {
    if (el.operation && el.operation === "delete") {
      ...
      if (!el.value) {
        ...
        scimgateway.logger.debug(
          `${pluginName}[${baseEntity}] handling "${action}" id=${id} deleted attrObj=${JSON.stringify(
            attrObj
          )}`
        );
      }
      ...
      if (el.value) {
        ...
        scimgateway.logger.debug(
          `${pluginName}[${baseEntity}] handling "${action}" id=${id} added member to group: attrObj=${JSON.stringify(
            attrObj
          )}`
        );
      }
