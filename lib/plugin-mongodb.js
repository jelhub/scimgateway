...
let dbname = config.dbname ? config.dbname : "scim";

const db = new mongoose(dbname, {
  env: "NODEJS",
  ...
  adapter:
    config.persistence === true
      ? new mongoose.LokiFsAdapter()
      : new mongoose.LokiMemoryAdapter(),
});
...
function loadHandler() {
  ...
  if (db.options.autoload === false) {
    // not using persistence (physical database) => load testusers
