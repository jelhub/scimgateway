...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
    ...
    switch (getObj.operator) {
      ...
      default:
        if (!validFilterOperators.includes(getObj.operator)) {
          const err = new Error(`filter operator '${getObj.operator}' is not valid, valid operators for this endpoint are: ${validFilterOperators}` + ',co,ge,le,sw,ew')
  ...
  if (!usersArr) throw new Error('mandatory if-else logic not fully implemented')

  if (!getObj.startIndex && !getObj.count) { // client request without paging
    getObj.startIndex = 1
    getObj.count = usersArr.length
  }
...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  if (!groupsArr) throw new Error('mandatory if-else logic not fully implemented')
  ...
  await attrObj.members.forEach(async el => {
    if (el.operation && el.operation === 'delete') { // delete member from group
      ...
      if (el.value) { // check if user exist
        ...
        const usrs = await scimgateway.getUsers(baseEntity, getObj, 'id')
        if (!usrs.Resources.length === 1 || !usrs.Resources[0].id === el.value) {
