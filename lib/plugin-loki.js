...
scimgateway.getUser = async (baseEntity, getObj, attributes) => {
  ...
  if (res.length === 1) return stripLoki(res[0]) // includes all user attributes but groups - user attribute groups automatically handled by scimgateway
  else return null // no user, or more than one user found
}
...
scimgateway.modifyUser = async (baseEntity, id, attrObj) => {
  ...
  if (typeof userObj === 'undefined') {
    ...
    for (var key in attrObj) {
      ...
      } else if (scimgateway.isMultivalue('User', key)) { // customized using type instead of array (e.g mails, phones, entitlements)
