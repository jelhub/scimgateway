...
function loadHandler () {
  ...
  scimgateway.getGroup = async (baseEntity, getObj, attributes) => {
    ...
    if (!attributes) return retObj
    else { // return according to attributes
      const ret = {}
      const arrAttr = attributes.split(',')
      for (let i = 0; i < arrAttr.length; i++) {
        const attr = arrAttr[i].split('.') // members.value
        if (retObj[attr[0]]) ret[attr[0]] = retObj[attr[0]]
      }
      if (JSON.stringify(ret) === '{}') return retObj // group with all attributes when specified attributes not found
      return ret
    }
  }
