...
scimgateway.getUsers = async (baseEntity, getObj, attributes) => {
  //
  // "getObj" = { attribute: <>, operator: <>, value: <>, rawFilter: <>, sortBy: <>, sortOrder: <>, startIndex: <>, count: <> }
  ...
  if (!soapRequest) throw new Error('mandatory if-else logic not fully implemented')
  ...
  try {
    ...
    if (!Array.isArray(result) || result.length < 4) {
      const err = new Error(`${config[soapAction].service}-${config[soapAction].method} : Invalid SOAP result: ${JSON.stringify(result)}`)
      throw err
    }
    ...
    if (!result.return) {
      const err = new Error(`${action} ${config[soapAction].method} : Got empty response on soap request: ${JSON.stringify(soapRequest)}`)
      throw err
    }
    ...
    if (hdl) {
      ...
      if (!Array.isArray(result) || result.length < 4) {
        const err = new Error(`${soapAction} searchPagedUser : Invalid SOAP result: ${JSON.stringify(result)}`)
        throw err
      }
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  try {
    ...
    if (!Array.isArray(result) || result.length < 4) {
      const err = new Error(`${config[action].service}-${config[action].method} : Invalid SOAP result: ${JSON.stringify(result)}`)
      throw err
    }
...
scimgateway.getGroups = async (baseEntity, getObj, attributes) => {
  ...
  try {
    ...
    if (!result) return ret // no groups
      ...
      const err = new Error(`${action} ${config[soapAction].method} : Got empty response on soap request: ${soapRequest}`)
      throw err
    }
  ...
  try {
    ...
    attrObj.members.forEach(async function (el) {
      if (el.operation && el.operation === 'delete') { // delete member from group
        ...
        if (!Array.isArray(result) || result.length < 4) {
          const err = new Error(`${config[action].service}-removeUserFromGroup : Invalid SOAP result: ${JSON.stringify(result)}`)
          throw err
        }
        result = result[0]

        if (!result.return) {
          const err = new Error(`${config[action].service}-removeUserFromGroup : Got empty response on soap request: ${soapRequest}`)
          throw err
        }
        return null
        ...
        if (!Array.isArray(result) || result.length < 4) {
          const err = new Error(`${config[action].service}-assignUserToGroup : Invalid SOAP result: ${JSON.stringify(result)}`)
          throw err
        }
        result = result[0]
