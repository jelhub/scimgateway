...
// Email        %UE% (Emails, type=Work)                emails.work.value               emailAddress
// Phone        %UP% (Phone Numbers, type=Work)         phoneNumbers.work.value         phoneNumber
// Company      %UCOMP% (Entitlements, type=Company)    entitlements.company.value      company
//
//=================================================================================
...
var dot = require('dot-object'); // Forwardinc
...
// baseEntity = Optional, used when multiple endpoint is needed (defined in base url e.g. <baseurl>/client1 gives baseEntity=client1)
// id         = user id
// attrObj    = object containing userattributes according to scim standard
//              Note, multi-value attributes excluding user attribute 'groups' are customized from array to object based on type
// callback   = null (OK) or error object
//==========================================
                ...
                for (let key1 in attrObj) {
                    if (typeof attrObj[key1] === 'object') { // name.familyName
                        for (let key2 in attrObj[key1]) {
                            dot.copy(`${key1}.${key2}`, `${key1}.${key2}`, attrObj, userObj)
                        }
                    }
                    else dot.copy(key1, key1, attrObj, userObj); // merge modified attr into userObj
                }
