...
scimgateway.modifyUser = async (baseEntity, id, attrObj) => {
  ...
  try {
    ...
    let userObj = await scimgateway.getUser(baseEntity, getObj, '')
...
const getServiceClient = async (baseEntity, action) => {
  if (!config.entity) {
    const err = new Error(`endpoint.entity is not defined in configuration file ${pluginName}.json`)
    throw err
  }
  if (!config.entity[baseEntity]) {
    const err = new Error(`endpoint.entity.${baseEntity} is not defined in configuration file ${pluginName}.json`)
    throw err
  }
  if (!config.entity[baseEntity][action]) {
    const err = new Error(`endpoint.entity.${baseEntity}.${action} is not defined in configuration file ${pluginName}.json`)
    throw err
  }
  const entityService = config.entity[baseEntity][action].service
  ...
  if (!config.entity[baseEntity].baseServiceEndpoint) config.entity[baseEntity].baseServiceEndpoint = config.entity[baseEntity].baseServiceEndpoints[0] // failover logic may set baseServiceEndpoint
  serviceEndpoint = config.baseServiceEndpoint + '/' + entityService

  serviceEndpoint = config.entity[baseEntity].baseServiceEndpoint + '/' + entityService
  // serviceEndpoint = config.entity[baseEntity].baseServiceEndpoint // some wsdl's needs this one instead of above

  const wsdlOptions = {}

  try {
    const serviceClient = await soap.createClientAsync(urlToWsdl, wsdlOptions)
    const endpointUsername = config.entity[baseEntity].username
    const endpointPassword = scimgateway.getPassword(`endpoint.entity.${baseEntity}.password`, configFile)
...
const doRequest = async (baseEntity, actionMethod, soapRequest, retryCount) => {
  try {
    ...
    if (method) {
      ...
      arrResult = await serviceClient[config.entity[baseEntity][action].method + 'Async'](soapRequest)
      result = arrResult[0]
      scimgateway.logger.debug(`${pluginName}[${baseEntity}] ${action} ${config.entity[baseEntity][action].method} Request = ${JSON.stringify(soapRequest)} Response = ${JSON.stringify(result)}`)
    }
    return result
  } catch (err) { // ECONNREFUSED or ENOTFOUND => retry/failover all baseServiceEndpoints starting with primary
    scimgateway.logger.error(`${pluginName}[${baseEntity}] doSoapRequest ${actionMethod} Request = ${JSON.stringify(soapRequest)} Error Response = ${err.message}`)
