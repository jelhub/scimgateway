...
const sqlPassword = scimgateway.getPassword('endpoint.connection.password', configFile)
...
scimgateway.exploreUsers = async (baseEntity, attributes, startIndex, count) => {
  ...
  connection.on('connect', function (err) {
    if (err) {
      const e = new Error(`Explore-Users connect: MSSQL client connect error: ${err.message}`)
      throw e
    }
...
scimgateway.createUser = async (baseEntity, userObj) => {
  ...
  connection.on('connect', function (err) {
    if (err) {
      const e = new Error(`Create-Users connect: MSSQL client connect error: ${err.message}`)
      throw e
    }
    ...
    const request = new Request(sqlQuery, function (err, rowCount, rows) {
      if (err) {
        connection.close()
        const e = new Error(`Create-Users: MSSQL client request: ${sqlQuery} Error: ${err.message}`)
        throw e
      }
      connection.close()
      return null
...
scimgateway.deleteUser = async (baseEntity, id) => {
  ...
  connection.on('connect', function (err) {
    if (err) {
      const e = new Error(`Delete-User connect: MSSQL client connect error: ${err.message}`)
      throw e
    }
    ...
    const request = new Request(sqlQuery, function (err, rowCount, rows) {
      if (err) {
        connection.close()
        const e = new Error(`Delete-User: MSSQL client request: ${sqlQuery} Error: ${err.message}`)
        throw e
      }
      connection.close()
      return null
...
scimgateway.modifyUser = async (baseEntity, id, attrObj) => {
  ...
  connection.on('connect', function (err) {
    if (err) {
      const e = new Error(`Modify-Users connect: MSSQL client connect error: ${err.message}`)
      throw e
    }
    ...
    const request = new Request(sqlQuery, function (err, rowCount, rows) {
      if (err) {
        connection.close()
        const e = new Error(`Modify-Users: MSSQL client request: ${sqlQuery} Error: ${err.message}`)
        throw e
      }
      connection.close()
      return null
